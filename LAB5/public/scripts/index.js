document.addEventListener('DOMContentLoaded', () => {
    ...
    async function loadInitialMovies() {
        ...
        if (movies.length < moviesPerLoad) {
             console.log("첫 페이지 로드 완료. 더 이상 로드할 영화가 없는 것으로 추정됩니다.");
        }
    }
    ...
    async function loadMoreMovies() {
        ...
        if (nextMovies.length > 0) {
            ...
            displayedMoviesCount += nextMovies.length;
             ...
             if (nextMovies.length < moviesPerLoad) {
                 console.log("추가 로드 완료. 더 이상 로드할 영화가 없는 것으로 추정됩니다.");
             }
    ...
    window.addEventListener('scroll', () => {
        ...
        if (isNearBottom && !isLoading && displayedMoviesCount > 0 && approxLastLoadedCount === moviesPerLoad) {
             loadMoreMovies();
        } else if (isNearBottom && !isLoading && displayedMoviesCount > 0 && approxLastLoadedCount < moviesPerLoad) {
             console.log("무한 스크롤: 마지막 영화들이 로드된 것으로 보입니다.");
        }
    });
